...
Pnet::Pnet(){
    ...
    string filename = filepath + "/Pnet.txt";
...
Rnet::Rnet(){
    ...
    cout << filepath << endl;
    string filename = filepath + "/Rnet.txt";
    readData(filename, dataNumber, pointTeam);
...
Onet::Onet(){
    ...
    string filename = filepath + "/Onet.txt";
    readData(filename, dataNumber, pointTeam);
...
mtcnn::mtcnn(int row, int col, std::string path){
    nms_threshold[0] = 0.7;
    nms_threshold[1] = 0.7;
    nms_threshold[2] = 0.7;
...
void mtcnn::findFace(const Mat& image, vector<Rect>& rects)
{
    struct orderScore order;
    int count = 0;
    ...
    for(vector<struct Bbox>::iterator it=thirdBbox_.begin(); it!=thirdBbox_.end();it++){
        if((*it).exist){
            rects.push_back(cv::Rect(it->y1, it->x1, it->y2-it->y1, it->x2-it->x1));
            //rectangle(image, Point((*it).y1, (*it).x1), Point((*it).y2, (*it).x2), Scalar(0,0,255), 2,8,0);
            //for(int num=0;num<5;num++)circle(image,Point((int)*(it->ppoint+num), (int)*(it->ppoint+num+5)),3,Scalar(0,255,255), -1);
        }
    }
